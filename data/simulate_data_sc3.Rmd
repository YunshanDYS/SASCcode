---
title: "Simulation data generation scenario 3"
author: "Yunshan Duan"
date: '2024-03-14'
...
```{r 2}
if (file.exists("./simulation_sc1/pbmc3k_final.rds")) {
  pbmc <- readRDS("./simulation_sc1/pbmc3k_final.rds")
} else {
  ...
  # pdf("./simulation/figures/elbowplot.pdf", width = 6, height = 6)
  ElbowPlot(pbmc)
  # dev.off()
  ...
  saveRDS(pbmc, file = "./simulation_sc1/pbmc3k_final.rds")
}
...
```{r 3}
# pbmc <- readRDS("./simulation/data/pbmc3k_final.rds")
...
ind_sel <- which(pbmc@meta.data$sc_type == "Naive CD4 T" | pbmc@meta.data$sc_type == "Memory CD4 T")
# count matrix
count <- as.matrix(pbmc@assays$RNA@counts)[, ind_sel]
dim(count)

# feature matrix after PCA (dim reduction)
gene <- pbmc@reductions$pca@cell.embeddings[ind_sel, ]
dim(gene)

# umap embeddings
gene_umap <- pbmc@reductions$umap@cell.embeddings[ind_sel, ]
dim(gene_umap)

# cell names
cell_names <- colnames(count)

cell_types <- pbmc@meta.data$sc_type[ind_sel]
...
gene_1 <- gene[which(cell_types == "Naive CD4 T"), ]
cluster_temp <- hclust(dist(gene_1))
cluster_1 <- cutree(cluster_temp, 2)
celltype_sim[which(cell_types == "Naive CD4 T")] <- cluster_1

gene_1 <- gene[which(cell_types == "Memory CD4 T"), ]
cluster_temp <- hclust(dist(gene_1))
cluster_1 <- cutree(cluster_temp, 2)
celltype_sim[which(cell_types == "Memory CD4 T")] <- cluster_1 + 2
condition_sim <- rep(NA, ncells)
for (ii in 1) {
  ind <- which(celltype_sim == ii)
  ind_LO <- sample(ind, round(length(ind)*0.58), replace = F)
...
for (ii in c(2,4)) {
  ind <- which(celltype_sim == ii)
  ind_LO <- sample(ind, round(length(ind)*0.05), replace = F)
  ind_YO <- ind[!ind %in% ind_LO]
  condition_sim[ind_LO] <- "B"
  condition_sim[ind_YO] <- "A"
}
for (ii in c(3)) {
  ind <- which(celltype_sim == ii)
  ind_LO <- sample(ind, round(length(ind)*0.9), replace = F)
  ind_YO <- ind[!ind %in% ind_LO]
  condition_sim[ind_LO] <- "B"
  condition_sim[ind_YO] <- "A"
}
...
celltype_CLE[which(celltype_sim %in% c(2,4))] <- "A"
celltype_CLE[which(celltype_sim %in% c(3))] <- "B"
celltype_CLE[which(celltype_sim %in% c(1))] <- "C"
...
```{r 5}
# sim_data <- readRDS("./simulation/data/sim_data.rds")
